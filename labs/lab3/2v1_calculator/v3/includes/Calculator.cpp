...
void Calculator::RunFnCommand(std::string& name, std::string& lValue, std::string& operation, std::string& rValue)
{
    ...
    if (operation == "" && rValue == "")
    {
        ...
        if (storage[lValue]->GetType() == Expression::ExpressionType::variable)
            PutInStorage(name, std::make_shared<Function>(name, 'v', storage[lValue]));
        else if (storage[lValue]->GetType() == Expression::ExpressionType::function)
            PutInStorage(name, std::make_shared<Function>(name, 'f', storage[lValue]));
        return;
    }
    ...
    if (operation != "" && rValue != "")
    {
        ...
        char lType = 'f';
        char rType = 'f';
        if (storage[lValue]->GetType() == Expression::ExpressionType::variable) lType = 'v';
        if (storage[rValue]->GetType() == Expression::ExpressionType::variable) rType = 'v';
        PutInStorage(name, std::make_shared<Function>(lValue, lType, rValue, rType, storage[lValue], storage[rValue], operation[0]));
        return;
    }
