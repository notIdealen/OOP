...
const uint32_t MAX_COLOR_VALUE = 0x00ff'ffff;
    ...
    uint32_t& oc, uint32_t& sc)
{
    ...
    if (IsValidNumber(args[1], x) &&
        ...
        IsValidColor(args[4], oc) && oc <= MAX_COLOR_VALUE &&
        IsValidColor(args[5], sc) && sc <= MAX_COLOR_VALUE)
    {
        return true;
    }
    return false;
...
bool IsValidTriangleArguments(const std::vector<std::string>& args, double& x1, double& y1, double& x2, double& y2, double& x3, double& y3, uint32_t& oc, uint32_t& sc)
{
    ...
    if (IsValidNumber(args[1], x1) &&
        ...
        IsValidColor(args[7], oc) && oc <= MAX_COLOR_VALUE &&  
        IsValidColor(args[8], sc) && sc <= MAX_COLOR_VALUE)
    {
        return true;
    }
    ...
    if (std::abs(crossProduct) < EPS) return false;
...
bool IsValidRectangleArguments(const std::vector<std::string>& args, double& l, double& t, double& w, double& h, uint32_t& oc, uint32_t& sc)
{
    ...
    if (IsValidNumber(args[1], l) &&
        ...
        IsValidColor(args[5], oc) && oc <= MAX_COLOR_VALUE &&  
        IsValidColor(args[6], sc) && sc <= MAX_COLOR_VALUE)
    {
        return true;
    }
    return false;
